...
function PostContent(props) {
	...
	const deletePost = useCallback(() => {
		...
		setTimeout(() => {
			const _posts = [...posts];
			const index = _posts.find(
				(element) => element.id === deletePost.id
			);
		...
		return (
			<Box m={2}>
				<HighlightedInformation>
					No posts added yet. Click on &quot;NEW&quot; to create
					your first one.
				</HighlightedInformation>
			</Box>
		);
	...
	return (
		<Paper>
			<Toolbar className={classes.toolbar}>
				<Typography variant="h4">POLICY DESIGNS</Typography>
