		...
		(async () => {
			const response = await fetch(
				item.id === 'select_insurance_product'
					...
					: item.id === 'country_present_in'
					? 'http://http://f2635d186ab8.ngrok.io/country'
		...
		<Autocomplete
			onChange={(event, values) =>
				item.id === 'select_insurance_product'
					...
					: (dataToSendToParent[item.id] = values.country_code)
			}
...
// export function AutoCompleteTextField({ item }) {
// 	return (
// 		<Autocomplete
// 			onChange={(values, value) => {
// 				dataToSendToParent[item.id] = value.country_code;

// 				console.log(dataToSendToParent[item.id]);
// 			}}
