...
function Main(props) {
	...
	const [hasFetchedEmojiTextArea, setHasFetchedEmojiTextArea] = useState(
		false,
	);
	const [ImageCropper, setImageCropper] = useState(null);
	const [hasFetchedImageCropper, setHasFetchedImageCropper] = useState(
		false,
	);
	...
	const [isAddBalanceDialogOpen, setIsAddBalanceDialogOpen] = useState(
		false,
	);
	...
	const fetchRandomStatistics = useCallback(() => {
		...
		let curUnix =
			Math.round(new Date().getTime() / 1000) - oneYearSeconds;
	...
	const selectDashboard = useCallback(() => {
		smoothScrollTop();
		document.title = 'XPCover - Dashboard';
	...
	const selectPosts = useCallback(() => {
		smoothScrollTop();
		document.title = 'XPCover - Posts';
		setSelectedTab('Posts');
		if (!hasFetchedEmojiTextArea) {
			setHasFetchedEmojiTextArea(true);
			import('../../shared/components/EmojiTextArea').then(
				(Component) => {
		...
		if (!hasFetchedImageCropper) {
			...
			import('../../shared/components/ImageCropper').then(
				(Component) => {
		...
		if (!hasFetchedDateTimePicker) {
			...
			import('../../shared/components/DateTimePicker').then(
				(Component) => {
	...
	const selectSubscription = useCallback(() => {
		smoothScrollTop();
		document.title = 'XPCover - Subscription';
